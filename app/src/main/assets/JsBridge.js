...
var Zepto = (function() {
    ...
    $.fn = {
        ...
        //DOM Ready
        ready: function(callback, jumpJsProxyInject) {
            var originCb = callback;
            var mcounter = 0;
            //尝试等待(1500ms超时)让壳注入JsProxy Js
            callback = function () {
                if(!window.JsProxy && mcounter++ < 150)setTimeout(callback, 10);else originCb($);
            };
            //是否跳过等待JsProxy的注入
            if (jumpJsProxyInject) {
                callback = originCb;
            }
...
(function($) {
    ...
    function serializeData(options) {
        ...
        //options.traditional表示是否以$.param方法序列化
            options.data = $.parameter(options.data, options.traditional)
    ...
    function serialize(parameters, obj, traditional, scope) {
        ...
        $.each(obj, function(key, value) {
            ...
            if (!scope && array) parameters.add(value.name, value.value)
            // recurse into nested objects
            //当value值是数组或者是对象且不是按传统的方式序列化的时候，需要再次遍历value
            else if (type == "array" || (!traditional && type == "object")) serialize(parameters, value, traditional, key)
            else parameters.add(key, value)
        })
    }
    //将obj转换为查询字符串的格式，traditional表示是否转换成传统的方式，至于传统的方式的意思看上面的注释
    $.parameter = function(obj, traditional) {
        var parameters = []
        //注意这里将add方法定到parameters，所以下面serialize时才不需要返回数据
        parameters.add = function(k, v) {
            this.push(escape(k) + '=' + escape(v))
        }
        serialize(parameters, obj, traditional)
